...
git diff --name-status --find-renames "$UPSTREAM_HEAD".."$LOCAL_BEFORE"
```

Save the merge base as `MERGE_BASE`. Treat commit history and each entry's `source_commits` as discovery evidence, never as proof that a local difference still exists. Exclude the ledger and this skill's own paths. For every unreviewed functional commit, inspect its patch, then compare the candidate behavior directly between `UPSTREAM_HEAD` and `LOCAL_BEFORE`:

```bash
git diff --find-renames "$UPSTREAM_HEAD".."$LOCAL_BEFORE" -- <candidate-paths>
git show "$UPSTREAM_HEAD:<path>"
git show "$LOCAL_BEFORE:<path>"
```

Create a candidate only when this snapshot comparison proves at least one behavior-specific net difference in implementation, configuration, defaults, API contract, or tests. A local-only commit, a changed path, or unrelated hunks in the same file do not qualify by themselves. If the commit was later reverted, upstream independently adopted the behavior, or both snapshots now implement the same observable behavior, omit it entirely; do not create an `excluded` entry. Reserve `excluded` for a genuine current local difference that the user explicitly declines to protect, with `review_reason` recorded.

For every surviving candidate, inspect its relevant tests before classifying it. Never create a feature entry from its commit subject or a top-level directory alone. Generate it with `status: pending_confirmation`, a stable `id`, category (`backend`, `ui`, or `ops`), exact paths, important symbols/callers, observable behavior, and focused verification. Group related candidates only when they form one observable product contract; preserve source commit/path evidence inside the grouped entry. Documentation-only, formatting-only, lockfile-only, and mechanical commits are not candidates unless they create a current behavior difference. Only entries with `status: approved` protect behavior during a merge.

When `bootstrap.state` is `required`, use `bootstrap.source_merge_base` as `LOCAL_FEATURE_BASE` after verifying it is an ancestor of `LOCAL_BEFORE`; do not replace it with the current merge base. If it is not an ancestor, stop and report the invalid bootstrap anchor. Discover candidates from `LOCAL_FEATURE_BASE..LOCAL_BEFORE`, filter them through the snapshot net-difference gate above, update the draft in the ledger, and present one compact approval table grouped by category. Use the default of protecting all functional groups. The user may reply `确认默认项` to approve all, or name only groups/IDs to exclude or amend. Mark excluded items as `status: excluded` with a `review_reason`; do not delete their source evidence. Re-run the net-difference gate before requesting confirmation. Do not merge, stage, or commit while any candidate is unconfirmed. After approval, persist reviewed entries, set `bootstrap.state: complete`, set `bootstrap.reviewed_local_head` to `LOCAL_BEFORE`, and make a separate focused commit for the ledger before restarting the sync from preflight. This one-time stop prevents an unreviewed historical fork from being silently overwritten even if the upstream merge base moves before bootstrap finishes.

On later syncs, verify `bootstrap.reviewed_local_head` exists and is an ancestor of `LOCAL_BEFORE`; stop for ledger repair if it is missing or history was rewritten. Derive new candidates from `bootstrap.reviewed_local_head..LOCAL_BEFORE`, not from the full fork history, then apply the same snapshot net-difference gate. Also revalidate every approved entry against `UPSTREAM_HEAD` and `LOCAL_BEFORE` at behavior level. If an entry's whole behavior is now identical upstream, remove the stale entry; if only part converged, prune only the matching behavior and paths. Do not use unrelated differences in a shared file as evidence that an entry remains local. This bookkeeping needs no feature decision because it changes no local behavior, but report it and commit the ledger separately before merging. Advance `reviewed_local_head` to `LOCAL_BEFORE` after the new range is reviewed. Keep the confirmation burden low: omit no-op and mechanical candidates, group coherent behavior, state the default, and accept `确认默认项` as a single approval. Never mark a candidate approved merely because it is adjacent to an approved feature.
