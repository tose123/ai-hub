...
export function HeroTerminalDemo(props: HeroTerminalDemoProps) {
  ...
  return (
    <div className={cn('mx-auto w-full max-w-2xl', props.className)}>
      <div
        className={cn(
          'border-border/80 overflow-hidden rounded-lg border bg-white/90 shadow-[var(--shadow-float)] backdrop-blur-xl'
        )}
      >
        {/* Tab strip */}
        <div
...
function renderResponseLine(line: string, demo: ApiDemoConfig): ReactNode {
  ...
  matches.forEach((match) => {
    ...
    if (placeholder === '<text>') {
      ...
      segments.push(
        <Muted key={`${placeholder}-${start}`}>{placeholder}</Muted>
      )
    }
