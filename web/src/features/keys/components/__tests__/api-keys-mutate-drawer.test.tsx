...
describe('API keys mutate drawer Auto group integration', () => {
  test('inherits the root Auto order and sends an empty override for every batch-created key', async () => {
    ...
    expect(document.body.textContent?.includes('Cross-group retry')).toBe(false)

    fireEvent.click(groupTrigger)
    const groupOptions = [
      ...document.querySelectorAll<HTMLElement>('[data-slot="command-item"]'),
    ]
    const firstGroupOption = groupOptions[0]
    if (!firstGroupOption) {
      throw new Error('Expected Auto to be the first group option')
    }
    expect(firstGroupOption).toHaveTextContent('Automatic routing')
    fireEvent.click(firstGroupOption)

    const autoOrderTrigger = findButton('Auto group order', true)
    expect(autoOrderTrigger).toHaveAttribute('aria-expanded', 'false')
    ...
    fireEvent.click(autoOrderTrigger)
    expect(autoOrderTrigger).toHaveAttribute('aria-expanded', 'true')
    expect(
      document.body.textContent?.includes(
  ...
  test('preserves an unsaved custom order and mode after Auto to ordinary to Auto changes', async () => {
    ...
    fireEvent.click(findButton('Auto group order', true))
    const autoOrderControl = document.querySelector<HTMLElement>(
      '[role="group"][aria-label="Auto group order"]'
    )
    if (!autoOrderControl) {
      throw new Error('Expected expanded Auto group order controls')
    }
    ...
    expect(restoredAutoOrderTrigger).toHaveAttribute('aria-expanded', 'false')
    fireEvent.click(restoredAutoOrderTrigger)
    expect(
      document.querySelector('button[aria-label="Remove vip"]')
    ).toBeTruthy()
    expect(document.body.textContent?.includes('1 / 3 groups selected')).toBe(
