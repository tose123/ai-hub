...
func TestRefundReservedTokenQuotaWithoutRedis(t *testing.T) {
	truncateTables(t)
	resetBatchUpdateTestState(t)

	token := createReserveTestToken(t, 20)
	reservation, err := TryReserveTokenQuotaWithReceipt(token.Id, token.Key, 7, false)
	require.NoError(t, err)
...
func TestRefundReservedTokenQuotaRestoresRedisBatchReservation(t *testing.T) {
	truncateTables(t)
	resetBatchUpdateTestState(t)
